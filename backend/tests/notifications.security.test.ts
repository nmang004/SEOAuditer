...
describe('Notifications Security', () => {
  ...
  it('should only allow userA to access their notifications', async () => {
    ...
    // Should not see each other's notifications (mock service returns empty, but real would be user-specific)
  });

  it('should reject invalid input for notification settings update', async () => {
    const res = await request(server)
